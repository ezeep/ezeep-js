...
| `@cortado-holding/colors`        | 1.1.12 → 2.0.5        | **Runtime (SCSS).** Verified 2.x is SCSS-compatible (`config.scss` identical, all imports resolve) and regenerated the local display-p3 patch against 2.0.5 (still required upstream). Patch re-apply round-trip confirmed. Kept pinned exact per [patches/README.md](patches/README.md). |
