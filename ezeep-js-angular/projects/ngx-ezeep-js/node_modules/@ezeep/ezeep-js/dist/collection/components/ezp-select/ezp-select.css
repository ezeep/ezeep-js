...
:host(:not(:last-child):not(.is-expanded)) {
  --wrap-separator-position-bottom: 0 0 0 0;
}

:host(.toggle-horizontal) {
  --label-align: start;
...
#wrap {
  ...
  box-shadow: var(--wrap-separator-position-top) var(--wrap-separator-color), var(--wrap-separator-position-right) var(--wrap-separator-color), var(--wrap-separator-position-bottom) var(--wrap-separator-color), var(--wrap-separator-position-left) var(--wrap-separator-color);
  transform: translateY(var(--wrap-translate-y));
  transition: transform var(--duration) var(--easing-out-quart), border-radius var(--duration) var(--easing-out-quart), box-shadow var(--duration) var(--easing-out-quart);
}
